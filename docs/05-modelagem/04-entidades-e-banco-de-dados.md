...
| 2.3 | Junho/2026 | Vinicius Soares | Tabelas `permissoes_perfil` e `sobrescritas_permissao` agora populadas e consultadas em runtime: seed automático no startup (`_garantir_permissoes_default`), endpoints CRUD expostos, permissões efetivas servidas via `/api/me/permissoes` e armazenadas em `sessionStorage` no frontend. |
